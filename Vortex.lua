-- VORTEX DOMINATOR V10.5 - TODAS AS FUNÇÕES RESTAURADAS
...
ListFrame.Parent = MainFrame; ListFrame.Size = UDim2.new(0.9, 0, 0, 80); ListFrame.Position = UDim2.new(0.05, 0, 0.82, 0)
ListFrame.BackgroundColor3 = Color3.fromRGB(10, 10, 10); UIList.Parent = ListFrame

local function createBtn(txt, pos, col)
    local b = Instance.new("TextButton", MainFrame)
    b.Text = txt; b.Size = UDim2.new(0.9, 0, 0, 33); b.Position = pos
...
-- 2. WORLD EATER (LENDÁRIO)
createBtn("WORLD EATER (LENDÁRIO)", UDim2.new(0.05, 0, 0.22, 0), Color3.fromRGB(255, 69, 0)).MouseButton1Click:Connect(function()
            ...
            for i = 1, 120 do
                for _, part in pairs(workspace:GetDescendants()) do
                    if part:IsA("BasePart") and not part.Anchored and not part:IsDescendantOf(game.Players.LocalPlayer.Character) then
                        part.CFrame = hrp.CFrame * CFrame.new(0, 2, 0); part.Velocity = Vector3.new(0, 100, 0)
                    end
                end
                task.wait(0.04)
            end
...
-- 3. EXPULSAR (POWER SPIN)
createBtn("EXPULSAR (POWER SPIN)", UDim2.new(0.05, 0, 0.29, 0), Color3.fromRGB(200, 0, 0)).MouseButton1Click:Connect(function()
        ...
        if p ~= lp and p.Character and p.Character:FindFirstChild("Head") and p.Character.Head:FindFirstChild("V9Tag") then
            ...
            if tHrp then
                for i = 1, 15 do
                    ...
                    hrp.Velocity = Vector3.new(15000, 15000, 15000); task.wait()
                end
            end
        end
    end
    hrp.CFrame = oldPos; HunterShield()
end)

-- 4. RADAR ELITE
createBtn("LIGAR RADAR ELITE", UDim2.new(0.05, 0, 0.36, 0), Color3.fromRGB(40, 40, 40)).MouseButton1Click:Connect(function()
    task.spawn(function()
            ...
            for _, p in pairs(game.Players:GetPlayers()) do
                if p ~= game.Players.LocalPlayer and p.Character and p.Character:FindFirstChild("HumanoidRootPart") then
                    if p.Character.HumanoidRootPart.Velocity.Magnitude > 165 and not suspeitos[p.Name] then
                        ...
                        local l = Instance.new("TextLabel", ListFrame); l.Size = UDim2.new(1, 0, 0, 25); l.Text = " [!] " .. p.Name; l.TextColor3 = Color3.new(1,1,1); l.BackgroundColor3 = Color3.fromRGB(40,0,0)
                    end
                end
            end
        end
...
-- 5. ESCUDO V8.4 HUNTER
local BpBtn = createBtn("ESCUDO VORTEX: OFF", UDim2.new(0.05, 0, 0.43, 0), Color3.fromRGB(150, 0, 0))
BpBtn.MouseButton1Click:Connect(function()
    ...
    if escudado then HunterShield() end
end)

-- 6. SINCRONIZAR / REVIVER
createBtn("SINCRONIZAR / REVIVER", UDim2.new(0.05, 0, 0.50, 0), Color3.fromRGB(0, 120, 120)).MouseButton1Click:Connect(HunterShield)

-- 7. VOO FANTASMA
createBtn("ATIVAR VOO FANTASMA", UDim2.new(0.05, 0, 0.57, 0), Color3.fromRGB(0, 50, 150)).MouseButton1Click:Connect(function()
    loadstring(game:HttpGet("https://rawscripts.net/raw/Universal-Script-Invinicible-Flight-R15-45414"))()
end)

-- 8. LIMPAR RADAR
createBtn("LIMPAR RADAR", UDim2.new(0.05, 0, 0.74, 0), Color3.fromRGB(80, 0, 0)).MouseButton1Click:Connect(function()
    for _, c in pairs(ListFrame:GetChildren()) do if c:IsA("TextLabel") then c:Destroy() end end
    suspeitos = {}
end)

-- [ LOOP DE PROTEÇÃO RENDERS ]
